---
title: "Simple, static maps with ggplot"
author: "M. Schrandt"
date: "5/24/2021"
output:
...
Today, we'll briefly cover how to get started with simple maps in ggplot. There are many ways to approach this, but I'll try to get you started. We'll make use of the r packages `tidyverse` (which contains `ggplot`), `sf`, and `ggspatial`.
...
We'll be using a dataset of fish count data for 7 species of reef fish. The data are from a fisheries-independent monitoring survey of polyhaline seagrass beds along the West Florida Shelf, from 2008 to 2018. These data are available on Mendeley Data: <http://dx.doi.org/10.17632/9bzshm5h46.1>
...
TBshore_sf_in <- read_sf(dsn = ".", layer = "tb_geo") %>%
  #just keep the geometry column since that's all we need for this exercise (it contains lat/lon)
  select(geometry) %>%
  st_make_valid()

# check coordinate reference system
st_crs(TBshore_sf_in)
```
The coordinate reference system (CRS) is set to NAD83. Location data are collected using GPS, which typically uses the WGS84 CRS (you'll want to check your unit to see how it records). I am going to change the CRS of the base layer so that our points are projecting in the same CRS.
...
```{r}
p3 <- ggplot() +
  geom_sf(data = TBshore_sf, fill = "darkolivegreen3", size = 0.1) +
  geom_sf(data = fishdat_sf, size = 1.5, aes(color = as.factor(Gear))) +
...
Example: Labeling boundaries/outlines. To reproduce the map (Figure 1) from Schrandt et al. 2021,  that these data are associated with, use the following code
```{r}
...
WIMap <- ggplot() +
  ...
  annotation_scale(location = "bl", width_hint = 0.25,
                   height = unit(0.1, "cm")) +
  #remove the background grid color
