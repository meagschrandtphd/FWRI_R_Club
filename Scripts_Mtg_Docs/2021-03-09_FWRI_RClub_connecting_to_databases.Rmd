...
Most likely, your data are not stored in R, especially if there's a lot of it. Best data practices indicate we should store data in a relational database (e.g., SQL, MS Access, Oracle, SAS can be used like a relational database).

In order to access the data and take advantage of all R has to offer (wrangling, visualizing, analyzing), you could query from your database and save as .csv files and then load into R...or you can connect R directly to your database. Connecting to the database can help with reproducibility of your work.

Here's an RStudio resource for working with databases in R: https://db.rstudio.com/

If you connect to the database, you can query your data with R language/functions or, sometimes, you can use the native language (e.g., SQL) in R and have R translate it on the backend.
...
Therefore, sometimes it can be beneficial to "collect" more to R and then process using R language.
...
And to query, you can use R functions, including the `tidyverse`

The `in_schema()` function has the syntax `in_schema(schema, table)`
...
This option is great if you already have a collection of working SQL code and you don't want to translate it into R.
